...
class DungeonBuilderSuite extends FunSuite:
  ...
  def makeRoom(
    ...
    Room(id = id, roomType = roomType, width = 8, height = 6, tiles = makeTiles(), entities = entities)
  ...
  test("exit doors no longer point to NEXT after wiring"):
    val dungeon = builder().build(totalRooms = 4).getOrElse(fail("build failed"))
    val allDoors = dungeon.rooms.values.flatMap(_.entities).collect { case d: Door => d }
    assert(
      allDoors.forall(_.targetRoomId != "NEXT"),
      s"Found unwired NEXT door: ${allDoors.filter(_.targetRoomId == "NEXT").map(_.id).mkString(", ")}"
    )

  test("entrance doors no longer point to PREV after wiring"):
    val dungeon  = builder().build(totalRooms = 4).getOrElse(fail("build failed"))
    val allDoors = dungeon.rooms.values.flatMap(_.entities).collect { case d: Door => d }
    assert(
      allDoors.forall(_.targetRoomId != "PREV"),
      s"Found unwired PREV door: ${allDoors.filter(_.targetRoomId == "PREV").map(_.id).mkString(", ")}"
    )

  test("exit door of first room points to a room that exists in the dungeon"):
    val dungeon   = builder().build(totalRooms = 4).getOrElse(fail("build failed"))
    val firstRoom = dungeon.currentRoom
    val exitDoors = firstRoom.entities.collect { case d: Door if d.direction == Direction.Down => d }
    assert(exitDoors.nonEmpty, "First room should have an exit door")
    assert(dungeon.rooms.contains(exitDoors.head.targetRoomId), "Exit door target should exist in dungeon")
  ...
  test("same seed produces the same dungeon structure"):
    val d1 = DungeonBuilder(testPool, Random(99L)).build(totalRooms = 4).getOrElse(fail("build 1 failed"))
    val d2 = DungeonBuilder(testPool, Random(99L)).build(totalRooms = 4).getOrElse(fail("build 2 failed"))
