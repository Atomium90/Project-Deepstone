...
class StateMachineSuite extends FunSuite:
  ...
  test("RewardChoice action routes through to RewardChoiceResolver.resolve"):
    ...
    val TransitionResult(next, _, _, events) = sm().applyActionPure(state, RewardChoice(Some(option.id)))
    val nextExp = next.asInstanceOf[ExplorationState]
    assertEquals(nextExp.player.equippedWeapon, Some(option))
  ...
  test("StateUpdate equipment carries an equipped item's setId and iconId through to ItemView"):
    ...
    val player = PlayerFixtures.startingPlayer(ClassId.Warrior)
      .copy(equippedArmor = Some(setArmor),
            equippedWeapon = Some(genericWeapon),
            equippedAccessories = Vector(Some(setAccessory), None)
      )
    val update = ExplorationState(player, simpleDungeon(), 1, 1).toStateUpdate()
